...
for i in {1..10}
do
	taskset --cpu-list $i sudo perf stat --cpu=$i -e task-clock,instructions,cycles,branch-instructions,branch-misses,cache-references,cache-misses -o ./outputs/for_loop/for_loop_$i.txt ./for_loop &
	taskset --cpu-list $i sudo perf stat --cpu=$i -e task-clock,instructions,cycles,branch-instructions,branch-misses,cache-references,cache-misses -o ./outputs/large_array/large_array_$i.txt ./large_array &
done
